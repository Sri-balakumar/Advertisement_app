...
class AdBannerScan(models.Model):
    _name = 'ad.banner.scan'
    _description = 'Ad Carousel Scan Log'
    _order = 'create_date desc'
    _rec_name = 'banner_id'

    banner_id = fields.Many2one(
        'ad.banner', string='Ad', required=True,
        ondelete='cascade', index=True)
    # create_date / create_uid are provided automatically — the scan timestamp
    # and (for public hits) the public user.
